...
public class RemoteRequest implements CancellableRunnable {
    ...
    public static Set<String> failedIds = new HashSet<String>();
            ...
            try {
                ...
                if (response.code() >= 300) {
                    ...
                    if(response.code() == 412 ){
                        ...
                        String errors = jObject.getString("error");
                        if(errors!=null && errors.equals("missing_stub") && failedIds.isEmpty()){
                            String reason = jObject.getString("reason");
                            String splitComma[] = reason.split(",");
                            String finalString = splitComma[0].substring(3);
                            failedIds.add(finalString);
                        }else{
                            return;
                        }
                        System.out.println("412 failed ids " + jsonData);
